 ...
 */
export async function clickAndWaitForWindow(bot: MyBot, slot: number, timeout = 3000, maxRetries = 2): Promise<boolean> {
 ...
 */
export async function clickAndWaitForUpdate(bot: MyBot, slot: number, timeout = 2000, maxRetries = 2): Promise<boolean> {
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
        if (areAHFlipsPending()) return false
...
export async function findAndClick(bot: MyBot, buttonName: string, opts: {
    ...
    const { waitForNewWindow: expectNewWindow = false, timeout = 3000, maxRetries = 2 } = opts
 ...
 */
export async function clickAndWaitForSign(bot: MyBot, slot: number, value: string, timeout = 3000, maxRetries = 2): Promise<boolean> {
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
        if (areAHFlipsPending()) return false
